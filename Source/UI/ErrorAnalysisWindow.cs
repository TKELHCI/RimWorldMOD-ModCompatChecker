...
namespace ModCompatChecker.UI
{
    public class ErrorAnalysisWindow : Window
    {
        ...
        public ErrorAnalysisWindow()
        {
            ...
            draggable = true;
            resizeable = true;
            layer = WindowLayer.Dialog;            TimeoutSeconds = ModCompatChecker.ModCompatMod.Instance?.Settings?.AnalysisTimeoutSeconds ?? 60;
            _needsRefresh = true;
        }
        ...
        private void StartAnalyze()
        {
            ...
            var mid = settings.IsCustomModel() ? settings.CustomModelId : mi.Id;            TimeoutSeconds = settings.EnableAnalysisTimeout ? settings.AnalysisTimeoutSeconds : 99999;
        ...
        private void DoFollowUp()
        {
            ...
            var thread = new Thread(() =>
            {
                try
                {
                    var result = AIService.CallAPIWithTimeout(                        ep.endpoint, settings.APIKey, mid, prompt, ep.provider, (settings.EnableAnalysisTimeout ? settings.AnalysisTimeoutSeconds : 99999), ref cancel);
