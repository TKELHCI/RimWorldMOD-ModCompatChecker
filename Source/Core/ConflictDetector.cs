...
namespace ModCompatChecker.Core
{
    ...
    public static class ConflictDetector
    {
        ...
        public static ConflictReport RunFullScan()
        {
            ...
            {
                // 3. 依赖与排序
                bool depEnabled = ModCompatChecker.ModCompatMod.Instance?.Settings?.EnableDependencyCheck ?? true;
                report.DependencyIssues = depEnabled ? DependencyChecker.Analyze(mods) : new List<DependencyIssue>();
            }
            catch (Exception ex)
            {
