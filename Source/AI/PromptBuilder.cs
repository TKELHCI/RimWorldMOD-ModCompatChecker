...
━━━━━━━━━━━━━━━━━━━━";
        ...
        public static string BuildHarmonyConflictPrompt(HarmonyConflict conflict, string language)
        {
            ...
            var gl = GetGlossary();
            if (gl.Length > 0) { sb.AppendLine(); sb.AppendLine(gl); }
            return sb.ToString();
        }
        ...
        /// <summary>加载内置+自定义术语表（每次实时读取，无缓存——文件小不影响性能）</summary>
        public static string GetGlossary()
        {
            var settings = ModCompatChecker.ModCompatMod.Instance?.Settings;
            if (settings == null || (!settings.EnableGlossary && !settings.EnableCustomGlossary))
                return "";

            var sb = new StringBuilder();
            string root = ModCompatChecker.ModCompatMod.Instance.Content.RootDir;
            string glossaryDir = System.IO.Path.Combine(root, "Glossary");

            if (settings.EnableGlossary)
            {
                string builtIn = System.IO.Path.Combine(glossaryDir, "BuiltInGlossary.txt");
                if (System.IO.File.Exists(builtIn))
                {
                    sb.AppendLine("=== RimWorld Modding Glossary ===");
                    sb.AppendLine(System.IO.File.ReadAllText(builtIn, System.Text.Encoding.UTF8));
                }
            }

            if (settings.EnableCustomGlossary)
            {
                string custom = System.IO.Path.Combine(glossaryDir, "CustomGlossary.txt");
                if (System.IO.File.Exists(custom))
                {
                    sb.AppendLine("=== User Custom Glossary ===");
                    sb.AppendLine(System.IO.File.ReadAllText(custom, System.Text.Encoding.UTF8));
                }
            }
