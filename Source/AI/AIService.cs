...
namespace ModCompatChecker.AI
{
    ...
    {
        private static System.Net.HttpWebRequest _currentRequest;
        public static string AnalyzeHarmonyConflict(
            HarmonyConflict conflict, string apiEndpoint, string apiKey,
            string modelId, ModelConfig.ApiProvider provider, int timeoutSeconds)
        {
            var prompt = PromptBuilder.BuildHarmonyConflictPrompt(conflict, PromptBuilder.GetPromptLanguage());
            bool cancel = false;
            return CallAPIWithTimeout(apiEndpoint, apiKey, modelId, prompt, provider, timeoutSeconds, ref cancel);
        }
        ...
        public static string AnalyzeError(
            ...
            ModelConfig.ApiProvider provider, int timeoutSeconds)
        {
            ...
            ModelConfig.ApiProvider provider, int timeoutSeconds)
        {
            ...
            return CallAPIWithTimeout(apiEndpoint, apiKey, modelId,
                "Say 'Connection OK' in one word.", provider, timeoutSeconds, ref cancel);
        }
            ...
            int timeoutSeconds, ref bool cancelFlag)
        {
            ...
            var request = (HttpWebRequest)WebRequest.Create(endpoint); System.Threading.Interlocked.Exchange(ref _currentRequest, request);
            request.Method = "POST";
            ...
            finally { System.Threading.Interlocked.Exchange(ref _currentRequest, null); }
        }


        public static void AbortCurrentRequest()
        {
            var req = System.Threading.Interlocked.Exchange(ref _currentRequest, null);
            try { req?.Abort(); } catch { }
        }

        private static void TryAbort(HttpWebRequest request)
        {
            try { request.Abort(); System.Threading.Interlocked.Exchange(ref _currentRequest, null); } catch { }
        }
